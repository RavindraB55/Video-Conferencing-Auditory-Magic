...
import * as sampletext from './Data/text-data.txt';
    ...
    return (
        <div className="App">
          <header className="App-header">
            ...
            //<div>Hello World</div>
            <div style={{
              height: '100vh', width: '100vw', maxHeight: '100vw', maxWidth: 'calc(100% - 50px)',
...

              {
                videoTracks.map((track, index) => {
                  ...
                  return (<Seat track={track} index={index} length={videoTracks.length} key={track.getId()}/>)
                  }
                )
              }
          ...
          <button onClick={() => videoTracks[0].mute()}>
            Video Off
          </button>
          <button onClick={() => toggleCamera(videoTracks[0].unmute())}>
          Video On
          </button>
          <div>Audio controls</div>
          <button onClick={() => audioTracks[0].mute()}>
            Video Off
          </button>
          <button onClick={() => toggleCamera(audioTracks[0].unmute())}>
            Video On
          </button>
          <div>
